...
from maze_rl.render.control_app import LabAppController
...
def test_app_controller_supports_baseline_current_ai_and_replay(tmp_path: Path) -> None:
    ...
    assert controller.last_state is not None
    assert controller.last_state["player_position"] == controller.last_result.final_player_position
    assert controller.last_state["monster_position"] == controller.last_result.final_monster_position
    assert controller.last_state["rendered_player_position"] == controller.last_result.final_player_position
    assert controller.last_state["rendered_monster_position"] == controller.last_result.final_monster_position
...
def test_app_controller_uses_mode_specific_checkpoint_directories(tmp_path: Path) -> None:
    ...
    model = create_model(training_config=TrainingConfig(checkpoint_dir=controller.active_checkpoint_dir), env=env)
    manager.save(model=model, episode=0, timesteps=0, training_summary={"episodes_seen": 0}, evaluation_summary={"episodes": 1})
...
def test_app_controller_compare_milestones_skips_missing_and_runs_existing(tmp_path: Path) -> None:
    ...
    assert any(item.checkpoint == "ckpt 0000" and item.status == "ok" for item in controller.compare_results)
    assert any(item.checkpoint == "ckpt 0050" and item.status == "ok" for item in controller.compare_results)
    assert any(item.checkpoint == "ckpt 0100" and item.status == "missing" for item in controller.compare_results)
...
def test_app_controller_training_controls_use_app_selected_mode(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_continue_training_from_latest(additional_episodes: int, checkpoint_dir: Path, training_mode: str, stop_event) -> None:
        _ = checkpoint_dir
    ...
    monkeypatch.setattr("maze_rl.render.control_app.continue_training_from_latest", fake_continue_training_from_latest)
...
def test_maze_learning_mode_simplifies_actions_and_slows_monster() -> None:
    ...
    assert maze_only.rows == 10
    assert maze_only.cols == 10
    assert maze_only.max_player_speed == 1
    assert maze_only.monster_speed == 1
    assert maze_only.monster_move_interval == 3
    assert maze_only.max_episode_steps == 120
    assert maze_only.stall_threshold == 40
    assert getattr(env.action_space, "n", None) == 4
    assert maze_only.reward.exit_progress_reward >= 2.0
...
def test_app_controller_builds_last_10_last_50_and_all_time_stats(tmp_path: Path) -> None:
    ...
    assert [(card.cycles, card.wins, card.losses) for card in cards] == [(10, 7, 3), (50, 30, 20), (60, 36, 24)]
